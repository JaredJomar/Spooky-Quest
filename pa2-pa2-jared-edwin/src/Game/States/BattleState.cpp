...
BattleState::BattleState(Player *player, Area *area)
{
    ...
    buttonChange.load("audio/ui/beep.wav");
    buttonChange.setVolume(0.5);
    buttonSelect.load("audio/ui/boop.wav");
...
void BattleState::update()
{
    ...
    {
        // Code Added
        if (dynamic_cast<Boss *>(enemy) != NULL && currentEnemyHealth <= 0 && dynamic_cast<Boss *>(enemy)->getTimesDefeated() == 0)
        {
            currentEnemyHealth = enemy->getmax_Health();
            dynamic_cast<Boss *>(enemy)->setTimesDefeated(dynamic_cast<Boss *>(enemy)->getTimesDefeated() + 1);
        }
